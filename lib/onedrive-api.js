const onedriveApi = {
	...
	signIn: () => {
		const AUTHSERVICE_URI = "https://login.microsoftonline.com/common/oauth2/v2.0/authorize";
	...
	_query: (query) => {
		...
		return new Promise((resolve, reject) =>
		{
			if (!!onedriveApi.accessToken)
			{
				...
				http(query.method, (query.url.startsWith("https://") ? query.url : MS_GRAPH_URL + query.url), query.headers, query.body).then(
					(response) => resolve(((query.parseResponseAsJson === false) ? response.response : JSON.parse(response.response))),
