...
pageSnippets.import = function (templateUrl, callback = null)
{
	...
	function _templateLoaded(url, xmlDocument)
	{
		...
		console.debug("snippets imported from " + url);
		_collectSinppets();
		_includeStylesheets();
		_includeScripts();
	};
		...
		for (let c = 0; c < sourceXml.childNodes.length; c += 1)
		{
			...
			case nodeType.element:
				...
				case "dht:call-function":
					// console.log("_appendNodes: call-function",xmlNode.getAttribute("name"), owner);
				...
				let postrenderFunction = xmlNode.getAttribute("dht:postrender");
					...
					{
				if (owner[postrenderFunction] !== undefined)
				{
						owner[postrenderFunction](child);
				}
				else
				{
