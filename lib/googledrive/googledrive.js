...
GoogleDrivePicker.prototype =
{
	...
	_doAuth: function (immediate, callback)
	{
		...
		let auth = gapi.auth.authorize(
		{
			...
			{ 
				// console.log("gapi.auth.authorize():", result);
				if (!!result.error)
				{
					this.onCancel?.();
					// owc.ui.notify("Please allow popups and retry.", "yellow");
					window.alert("Please allow popups and retry.");
					console.error('gapi.auth.authorize() =>', result.error);
				}
				else
				{
					callback(result);
